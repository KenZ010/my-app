...
// Empty form for adding new supplier
const emptyForm = {
  code: "", itemName: "", supplierName: "", contactNo: "",
  lastOrdered: 0, lastCheckBy: "", dateChecked: "", status: "Active",
};
...
export default function SupplierMaintenancePage() {
  ...
  // Save add or edit
  const handleSave = () => {
    if (!form.code || !form.itemName) {
      alert("Code and Item Name are required.");
      return;
    }
    if (editingId !== null) {
      setItems((prev) =>
  ...
  const handleExport = () => {
    ...
    const rows = items.map((item) => [
      item.code, item.itemName, item.supplierName, item.contactNo,
      item.lastOrdered, item.lastCheckBy, item.dateChecked, item.status,
    ]);
    ...
    <div className="flex min-h-screen bg-gray-50 font-sans">
      ...
      <main className="flex-1 flex flex-col overflow-auto">
        ...
        <header className="flex items-center justify-between px-6 py-4 bg-white border-b border-gray-100">
          <h1 className="text-2xl font-bold text-gray-800">Supplier Maintenance</h1>
        ...
        <div className="flex-1 p-4 bg-green-50">
          <div className="bg-white rounded-2xl p-4 shadow-sm">
            ...
            <div className="flex items-center gap-2 mb-4 flex-wrap justify-end">
              ...
              <button onClick={openAddModal} className="flex items-center gap-1 bg-gray-900 rounded-lg px-3 py-2 text-sm text-white hover:bg-gray-700">+ Add Supplier</button>
            </div>
                ...
                <tbody>
                  {filtered.map((row) => (
                    <tr key={row.id} className={`border-b border-gray-100 hover:bg-gray-50 ${selected.includes(row.id) ? "bg-indigo-50" : ""}`}>
                      ...
                      <td className="p-3">
                        <span className="bg-gray-700 text-white px-3 py-1 rounded-full text-xs">{row.supplierName}</span>
                      </td>
                      <td className="p-3 text-gray-700">{row.contactNo}</td>
                      <td className="p-3 text-gray-700">{row.lastOrdered}</td>
                      <td className="p-3">
                        <span className="bg-gray-100 text-gray-600 px-2 py-1 rounded-full text-xs">{row.lastCheckBy}</span>
                      </td>
                      <td className="p-3">
                        <span className="bg-gray-100 text-gray-600 px-2 py-1 rounded-full text-xs">{row.dateChecked}</span>
                      </td>
                      <td className="p-3">
                        <span className={`px-3 py-1 rounded-full text-xs font-medium ${
                          row.status === "Active" ? "bg-green-500 text-white" : "bg-yellow-400 text-black"
                        }`}>
                          {row.status}
                        </span>
                      </td>
            ...
            <div className="flex flex-col gap-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input type="number" value={form.lastOrdered} onChange={(e) => setForm({ ...form, lastOrdered: Number(e.target.value) })}
                    className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm mt-1 outline-none focus:border-indigo-400 text-gray-900" />
                </div>
                <div>
                  <label className="text-xs font-medium text-gray-600">Date Checked</label>
                  <input value={form.dateChecked} onChange={(e) => setForm({ ...form, dateChecked: e.target.value })}
                    className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm mt-1 outline-none focus:border-indigo-400 text-gray-900" placeholder="e.g. 07 Apr 2030" />
                </div>
              </div>
              <div>
                <label className="text-xs font-medium text-gray-600">Last Check By</label>
                <input value={form.lastCheckBy} onChange={(e) => setForm({ ...form, lastCheckBy: e.target.value })}
                  className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm mt-1 outline-none focus:border-indigo-400 text-gray-900" />
              </div>
              <div>
