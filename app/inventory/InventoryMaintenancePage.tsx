...
const emptyForm = {
  code: "", name: "", date: "", total: 0, remaining: 0,
  lastCheck: "", expiry: "Fresh/ Valid", expiryColor: "green",
  stock: "In Stock", stockColor: "green",
};
...
export default function InventoryMaintenancePage() {
  ...
  const topSellingData = items.slice(0, 5).map((item) => ({
    name: item.name,
    units: item.total,
  }));
  ...
  const openEditModal = () => {
    ...
    if (!item) return;
    setForm({
      code: item.code, name: item.name, date: item.date,
      total: item.total, remaining: item.remaining,
      lastCheck: item.lastCheck, expiry: item.expiry,
      expiryColor: item.expiryColor, stock: item.stock,
  ...
  const handleExport = () => {
    ...
    const rows = items.map((item) => [
      item.code, item.name, item.date, item.total, item.remaining,
      item.lastCheck, item.expiry, item.stock,
    ]);
    ...
    <div className="flex min-h-screen bg-gray-50 font-sans">
      ...
      <main className="flex-1 flex flex-col overflow-auto">
        ...
        <header className="flex items-center justify-between px-6 py-4 bg-white border-b border-gray-100">
          <h1 className="text-2xl font-bold text-gray-800">Inventory Maintenance</h1>
        ...
        <div className="flex-1 p-4 bg-green-50">
          <div className="bg-white rounded-2xl p-4 shadow-sm mb-4">
            <div className="flex items-center gap-2 mb-4 flex-wrap">
              ...
              <button className="flex items-center gap-1 border border-gray-200 rounded-lg px-3 py-2 text-sm text-gray-600 hover:bg-gray-50">👤 Category ▾</button>
              ...
              <button className="flex items-center gap-1 border border-gray-200 rounded-lg px-3 py-2 text-sm text-gray-600 hover:bg-gray-50">📅 Date ▾</button>
              <button onClick={handleExport} className="flex items-center gap-1 border border-gray-200 rounded-lg px-3 py-2 text-sm text-gray-600 hover:bg-gray-50 ml-auto">📤 Export</button>
              <button onClick={handleDelete} className="flex items-center gap-1 border border-red-200 rounded-lg px-3 py-2 text-sm text-red-500 hover:bg-red-50">🗑️ Delete</button>
              <button onClick={openEditModal} className="flex items-center gap-1 border border-gray-800 rounded-lg px-3 py-2 text-sm text-gray-800 hover:bg-gray-100">✏️ Edit</button>
              <button onClick={openAddModal} className="flex items-center gap-1 bg-gray-900 rounded-lg px-3 py-2 text-sm text-white hover:bg-gray-700">+ Add Item</button>
            </div>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-indigo-900 text-white text-xs">
                    ...
                    <th className="p-3 text-left">Expiry Status</th>
            ...
            <div className="flex flex-col gap-3">
              ...
              <div>
                ...
                <input value={form.date} onChange={(e) => setForm({ ...form, date: e.target.value })}
                  className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm mt-1 outline-none focus:border-indigo-400 text-gray-900" placeholder="e.g. 06 Nov 2025" />
              </div>
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <input type="number" value={form.remaining} onChange={(e) => setForm({ ...form, remaining: Number(e.target.value) })}
                    className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm mt-1 outline-none focus:border-indigo-400 text-gray-900" />
                </div>
              </div>
              <div>
                <label className="text-xs font-medium text-gray-600">Last Check By</label>
                <input value={form.lastCheck} onChange={(e) => setForm({ ...form, lastCheck: e.target.value })}
                  className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm mt-1 outline-none focus:border-indigo-400 text-gray-900" />
              </div>
              <div>
                <label className="text-xs font-medium text-gray-600">Expiry Status</label>
                <select value={form.expiry} onChange={(e) => {
                  const val = e.target.value;
                  const colorMap: Record<string, string> = {
                    "Fresh/ Valid": "green", "Expired": "red", "Expiring Soon": "orange", "No Expiry": "blue", "Unknown": "gray",
                  };
                  setForm({ ...form, expiry: val, expiryColor: colorMap[val] || "gray" });
                }} className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm mt-1 outline-none focus:border-indigo-400 text-gray-900">
                  <option>Fresh/ Valid</option>
              ...
              <div>
                ...
                <select value={form.stock} onChange={(e) => {
                  const val = e.target.value;
                  const colorMap: Record<string, string> = { "In Stock": "green", "Out of Stock": "red", "Low Stock": "yellow" };
                  setForm({ ...form, stock: val, stockColor: colorMap[val] || "green" });
                }} className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm mt-1 outline-none focus:border-indigo-400 text-gray-900">
        ...
        <div className="fixed inset-0 bg-black bg-opacity-10 flex items-center justify-center z-50">
          <div className="bg-white rounded-2xl p-6 w-80 shadow-xl text-center">
            ...
            <p className="text-sm text-gray-500 mb-5">
              Are you sure you want to delete {selected.length} item(s)? This cannot be undone.
            </p>
