...
const filePath = path.join(__dirname, 'app', 'purchase-order', 'PurchaseOrderPage.tsx');
...
// 5. Add employeeId to the API call
content = content.replace(
  '        notes: form.notes || "",\n        items:',
  '        notes: form.notes || "",\n        employeeId,\n        items:'
);

// 6. Remove Delivery Date section from Order Preview (form.deliveryDate && (...))
content = content.replace(
  /\s*\{form\.deliveryDate && \(\s*<div className="mb-4 flex items-center gap-2 p-2\.5 bg-gray-50 rounded-xl border border-gray-100">[\s\S]*?</div>\s*\)\}/,
  ''
);

// 7. Remove DateField component from create form
content = content.replace(
  /\s*\{\/\* ✅ Custom date field — calendar icon, readable date ✅ \*\/\}\s*<DateField[\s\S]*?\/>/,
  ''
);
