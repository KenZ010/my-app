...
# Find and remove the entire form.deliveryDate && (...) block
date_section_pattern = r'\s*\{form\.deliveryDate && \(\s*<div className="mb-4 flex items-center gap-2 p-2\.5 bg-gray-50 rounded-xl border border-gray-100">.*?</div>\s*\)\}'
content = re.sub(date_section_pattern, '', content, flags=re.DOTALL)

# 7. Remove DateField component
datefield_pattern = r'\s*\{\/\* ✅ Custom date field — calendar icon, readable date ✅ \*\/\}\s*<DateField[\s\S]*?\/>'
content = re.sub(datefield_pattern, '', content)
